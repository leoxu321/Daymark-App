...
export function MonthlyGoalCalendar() {
  ...
  const calendarDays = useMemo(() => {
    ...
    while (current <= endDate) {
      ...
      const metGoal = isPastOrToday && completed >= goal
  ...
  const monthlyStats = useMemo(() => {
    ...
    const daysWithActivity = currentMonthDays.filter(d => d.completed > 0)
    const daysMetGoal = currentMonthDays.filter(d => d.metGoal)
    const totalCompleted = currentMonthDays.reduce((sum, d) => sum + d.completed, 0)
    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">Application Streak</CardTitle>
      ...
      <CardContent>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  day.metGoal && 'bg-green-100 dark:bg-green-900/30',
                  isPast && !day.metGoal && hasActivity && 'bg-yellow-100 dark:bg-yellow-900/30',
                  isPast && !hasActivity && day.isCurrentMonth && 'bg-red-50 dark:bg-red-900/20'
                )}
                title={`${day.date}: ${day.completed}/${day.goal} applications`}
              >
                <span className={cn(
                  'font-medium',
        ...
        <div className="mt-4 pt-3 border-t flex justify-between text-sm">
          ...
          <div className="text-muted-foreground">
            <span className="font-medium text-foreground">{monthlyStats.totalCompleted}</span> applications
          </div>
        </div>
      </CardContent>
    </Card>
